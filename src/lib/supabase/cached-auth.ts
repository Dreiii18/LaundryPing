...
export const getCachedUser = cache(async () => {
  ...
  const { data: laundromat, error: laundromatError } = await supabase
    .from('laundromats')
    .select('id, name, address, sms_free_credits, sms_paid_credits, billing_cycle_start, available_services, service_prices, service_weights, rush_fee, contact_number, receipt_paper_size')
    .eq('user_id', user.id)
    .single();
